...
const AdminCopyTrades = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    leverage: 50,
    price: 500,
    last10Trades: Array.from({ length: 10 }).map(() => ({ isWin: true, pnl: 10 })) as TradeDetail[]
  });

  // Load traders from localStorage or defaults
  useEffect(() => {
    ...
    if (savedTraders) {
      ...
      setTraders(defaults);
      localStorage.setItem('admin_copy_traders', JSON.stringify(defaults));
    }
    setIsLoading(false);
  }, []);

  const saveToStorage = (newTraders: CopyTradeType[]) => {
    setTraders(newTraders);
    localStorage.setItem('admin_copy_traders', JSON.stringify(newTraders));
  };
  ...
  const updateTradeHistory = <K extends keyof TradeDetail>(index: number, field: K, value: TradeDetail[K]) => {
    ...
    // Ensure PnL matches isWin status visually for the admin
    if (field === 'isWin') {
      const currentPnl = Math.abs(newHistory[index].pnl);
      newHistory[index].pnl = value ? currentPnl : -currentPnl;
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newTrader: CopyTradeType = {
      id: editingTrader ? editingTrader.id : `t-${Date.now()}`,
      traderName: formData.traderName,
      countryCode: formData.countryCode,
      coinSymbol: formData.coinSymbol.toUpperCase(),
      coinIcon: `cryptocurrency-color:${formData.coinSymbol.toLowerCase()}`,
      leverage: formData.leverage,
      price: formData.price,
      last10Trades: formData.last10Trades
    };

    if (editingTrader) {
      const updated = traders.map(t => t.id === editingTrader.id ? newTrader : t);
      saveToStorage(updated);
      toast.success('Trader updated');
    } else {
      saveToStorage([newTrader, ...traders]);
      ...
      <div className="max-w-7xl mx-auto">
              ...
              <tbody>
                {filteredTraders.length > 0 ? (
                  filteredTraders.map((trader) => {
                    ...
                    const winRate = Math.round((wins / trader.last10Trades.length) * 100);
                    
                    return (
                      <tr key={trader.id} className="border-b border-[#222] hover:bg-[#151515] transition-colors">
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-xl bg-[#1a1a1a] flex items-center justify-center overflow-hidden border border-[#222] relative">
                        ...
                        <td className="p-4">
                          <div className="flex items-center gap-2">
                            <div className={`text-xs font-bold ${winRate >= 50 ? 'text-[#00ff55]' : 'text-red-500'}`}>
                              {winRate}%
                            </div>
                        ...
                        <td className="p-4">
                          <div className="flex justify-end gap-2">
                            <button 
                              onClick={() => handleOpenEdit(trader)}
                              className="p-2 hover:bg-[#222] rounded-lg text-gray-400 hover:text-[#ebb70c] transition-colors"
                            >
                              <Edit size={18} />
                            </button>
                            <button 
                  ...
                  <tr>
                    <td colSpan={6} className="p-12 text-center text-gray-500">
                      {isLoading ? 'Loading traders...' : 'No traders found matching your search.'}
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative bg-[#111] border border-[#222] w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl overflow-hidden shadow-2xl custom-scrollbar">
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-5">
                    <div className="col-span-2">
                      ...
                      <div className="relative">
                        <Icon icon="ph:user-bold" className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 w-5 h-5" />
                        <input 
                          required
                          type="text"
                          value={formData.traderName}
                          onChange={(e) => setFormData({...formData, traderName: e.target.value})}
                          className="w-full bg-[#0a0a0a] border border-[#2a2a2a] rounded-xl py-3 pl-12 pr-4 text-white focus:outline-none focus:border-[#ebb70c] focus:ring-1 focus:ring-[#ebb70c] transition-all"
                    ...
                    <div className="col-span-2 sm:col-span-1">
                      <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">Country Code (2 letters)</label>
                      <div className="relative">
                        <div className="absolute left-4 top-1/2 -translate-y-1/2 flex items-center justify-center pointer-events-none">
                          <Icon icon={`circle-flags:${formData.countryCode ? formData.countryCode.toLowerCase() : 'us'}`} className="w-5 h-5 rounded-full shadow-sm" />
                        </div>
                        <input 
                          required
                          type="text"
                          ...
                          className="w-full bg-[#0a0a0a] border border-[#2a2a2a] rounded-xl py-3 pl-12 pr-4 text-white focus:outline-none focus:border-[#ebb70c] focus:ring-1 focus:ring-[#ebb70c] transition-all uppercase"
                    ...
                    <div className="col-span-2 sm:col-span-1">
                      ...
                      <div className="relative">
                        <Icon icon="ph:currency-btc-bold" className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 w-5 h-5" />
                        <input 
                          required
                          type="text"
                ...
                <div className="bg-[#151515] border border-[#222] p-6 rounded-2xl space-y-5">
                  ...
                  <div className="grid grid-cols-2 gap-5">
                    <div>
                      <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">Max Leverage</label>
                      <div className="relative">
                        <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 font-bold text-sm">x</span>
                        <input 
                          required
                          type="number"
                          value={formData.leverage}
                          onChange={(e) => setFormData({...formData, leverage: parseInt(e.target.value)})}
                          className="w-full bg-[#0a0a0a] border border-[#2a2a2a] rounded-xl py-3 pl-10 pr-4 text-white focus:outline-none focus:border-[#ebb70c] focus:ring-1 focus:ring-[#ebb70c] transition-all"
                        />
                      </div>
                    </div>
                ...
                <div className="bg-[#151515] border border-[#222] p-6 rounded-2xl space-y-5">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="bg-[#00ff55]/10 text-[#00ff55] border border-[#00ff55]/20 font-bold text-[10px] uppercase tracking-wider px-3 py-1 rounded-full flex items-center gap-1.5">
                      <div className="w-1.5 h-1.5 bg-[#00ff55] rounded-full animate-pulse" />
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                    {formData.last10Trades.map((trade, idx) => (
                      <div key={idx} className="bg-[#0a0a0a] border border-[#2a2a2a] p-3 rounded-xl space-y-3 relative group hover:border-[#444] transition-colors">
                        <div className="flex justify-between items-center">
                          <span className="text-[10px] text-gray-500 font-bold">#{idx + 1}</span>
                          <button
                            type="button"
                            onClick={() => updateTradeHistory(idx, 'isWin', !trade.isWin)}
                            className={`w-6 h-6 rounded-md flex items-center justify-center transition-all ${
                              trade.isWin ? 'bg-[#00ff55]/20 text-[#00ff55]' : 'bg-red-500/20 text-red-500'
                            }`}
                          >
                            <Icon icon={trade.isWin ? "ph:trend-up-bold" : "ph:trend-down-bold"} className="w-3.5 h-3.5" />
                          </button>
                        </div>
                        <div className="relative flex items-center justify-between bg-[#151515] rounded-lg border border-[#222] focus-within:border-[#ebb70c] transition-colors p-0.5 sm:p-1">
                          <button
                            type="button"
                            onClick={() => updateTradeHistory(idx, 'pnl', Number((Math.max(0, Math.abs(trade.pnl) - 0.5)).toFixed(1)))}
                            className="w-6 h-6 sm:w-7 sm:h-7 flex shrink-0 items-center justify-center bg-[#222] hover:bg-[#333] rounded text-gray-400 active:scale-95 transition-all"
                          >
                            <Icon icon="ph:minus-bold" className="w-3 h-3" />
                          </button>
                          
                          <div className="flex items-center justify-center flex-1 min-w-0 px-1">
                            <span className={`text-[10px] font-bold shrink-0 ${trade.isWin ? 'text-[#00ff55]' : 'text-red-500'}`}>
                              {trade.isWin ? '+' : '-'}
                            </span>
                            <input 
                              type="number"
                              ...
                              onChange={(e) => {
                                ...
                                updateTradeHistory(idx, 'pnl', isNaN(val) ? 0 : val);
                              }}
                              className={`w-full min-w-[20px] max-w-[36px] bg-transparent text-center text-[11px] sm:text-xs font-bold py-1 outline-none m-0 ${
                                trade.isWin ? 'text-[#00ff55]' : 'text-red-500'
                          ...
                          <button
                            type="button"
                            onClick={() => updateTradeHistory(idx, 'pnl', Number((Math.abs(trade.pnl) + 0.5).toFixed(1)))}
                            className="w-6 h-6 sm:w-7 sm:h-7 flex shrink-0 items-center justify-center bg-[#222] hover:bg-[#333] rounded text-gray-400 active:scale-95 transition-all"
                          >
                ...
                <div className="pt-4">
                  <button 
                    type="submit"
                    className="w-full bg-gradient-to-r from-[#ebb70c] to-[#f3cb3d] hover:from-[#d4a40b] hover:to-[#ebb70c] text-black font-bold py-4 rounded-xl transition-all active:scale-[0.98] shadow-[0_4px_20px_rgba(235,183,12,0.3)] flex items-center justify-center gap-2 text-sm uppercase tracking-wider"
                  >
                    <Icon icon={editingTrader ? "ph:check-circle-bold" : "ph:rocket-launch-bold"} className="w-5 h-5" />
                    {editingTrader ? 'Commit Updates' : 'Deploy Trader to Dashboard'}
                  </button>
                </div>
              </form>
            </div>
