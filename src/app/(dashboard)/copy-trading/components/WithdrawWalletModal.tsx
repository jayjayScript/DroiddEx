...
const WithdrawWalletModal = ({ isOpen, onClose, onWithdraw, copyWalletBalance }: WithdrawWalletModalProps) => {
      ...
      <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[90%] max-w-sm bg-[#111111] border border-[#3A3A3A] rounded-2xl p-6 z-[9999] shadow-2xl">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-[16px] font-bold text-white flex items-center gap-2">
            <Icon icon="ph:arrow-circle-up-right-fill" className="text-[#ebb70c]" /> Withdraw to WEB4
          </h2>
        ...
        <div className="space-y-6">
          <div className="bg-[#1a1a1a] rounded-xl p-4 border border-[#222]">
            <div className="flex justify-between items-center mb-1">
              <span className="text-gray-400 text-[11px] font-bold uppercase tracking-wider">To</span>
              <span className="text-white font-bold text-sm">WEB4 Wallet</span>
            </div>
            <p className="text-[#ebb70c] text-right font-bold text-xs">
              Copy Balance: ${copyWalletBalance.toLocaleString(undefined, { minimumFractionDigits: 2 })}
            </p>
          </div>

          <div>
            <label className="block text-[12px] font-bold text-gray-400 mb-2 uppercase tracking-wider">Amount to Withdraw</label>
          ...
          <button
            ...
            disabled={!amount || parseFloat(amount) <= 0 || parseFloat(amount) > copyWalletBalance}
